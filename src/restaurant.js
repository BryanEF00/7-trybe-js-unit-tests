  ...
  const restaurant = {
    ...
    pay: () => {
      ...
      for (let index = 0; index < restaurant.consumption.length; index += 1) {
        const requestedItem = restaurant.consumption[index];
        if (object.food[requestedItem] || object.drink[requestedItem]) {
          totalAmount += object.food[requestedItem] || object.drink[requestedItem];
        }
      }
